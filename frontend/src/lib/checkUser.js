...
export const checkUser = async () => {
  ...
  //Check if the user has pro plan
  const { has } = await auth();
  const subscriptionTier = has({ plan: "pantry_pro" }) ? "pro" : "free";
  ...
  try {
    ...
    if (existingUserData.length > 0) {
      const exisitingUser = existingUserData[0];

      //Update subcription Tier if changed
